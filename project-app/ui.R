...
takeaways <- tabPanel(
  ...
  p("The increase in fires that is shown by the US maps as time progresses supports the
    ...
    in the surface temperature data. With the increase in overall temperatures and the
...
introduction <- tabPanel("Introduction",
    ...
    p("We hope to frame our project around data goals. We want to use data gathered about
      exoplanets to find patterns in how features in one may reflect features of the other
...
brief_intro <- tabPanel("Project Brief & Intro",
  ...
  p(strong("Authors:"), "KelliAnn Ramirez (kellianr@uw.edu),
    Salley Fang (skfang@uw.edu), Claire Zhang (czhang88@uw.edu)"),
  ...
  p(strong("Keywords:"), "Shaping the future, Space exploration, Stars and exoplanets"),
  h3("Introduction"),
...
problem_questions <- tabPanel("Problem Domain & Research Questions",
  h2("Problem Domain"),
  p("Our topic is about outer space, specifically how we can discover and study exoplanets
                     and their surroundings to determine if they are habitable. Our topic concerns human
                     well-being, as the state of Earth may affect the ability for humans to inhabit it in the
  ...
  p("We hope to frame our project around data goals. We want to use data
                     gathered about exoplanets to find patterns in how features in
...
datasets <- tabPanel("The Datasets",
  h3("The Datasets"),
  p("Climate change and unusual weather conditions is what motivates us to think about what’s
                     ...
                     for humans."),
  tags$a(href="https://www.kaggle.com/datasets/sevgisarac/temperature-change?select=Environment_Temperature_change_E_All_Data_NOFLAG.csv", 
         "Earth Temperatures"),
  p("This dataset was obtained from kaggle and we fully intend on crediting the source. The dataset
    ...
    from the federal government. For Sevgi Sy, they offered their own time to clean and put this
    dataset on kaggle. Sevgi Sy will likely make money from the dataset that’s on kaggle, but if
    people were to go to the NASA GISTEMP site and download it there, then NASA GISTEMP would be
    making money. The data is trustworthy because it is collected and published through NASA, but
    we were unable to find further information on the author of the dataset we used, Sevgi Sy."),
  tags$a(href="https://www.kaggle.com/datasets/sathyanarayanrao89/nasa-exoplanetary-system", 
         "Exoplanets"),
  p("This dataset was obtained from kaggle and we fully intend on crediting the source. The dataset
    ...
    on kaggle. Sathyanarayan Rao is likely to make money from the dataset that’s on kaggle, but if people
    were to go to the NASA site and download it there, then NASA would be making money. The data is likely
    trustworthy because it’s NASA’s data, but we were unable to find further information
    on the author of the dataset we used."),
  tags$a(href="https://www.kaggle.com/datasets/chidmuthu/fires-clean", 
  ...
  h3("Dataset Table"),
  tableOutput("dataset_df"),
  h3("Citations"),
  p("Food and Agricultural Organization (FAOSTAT). (2020). Temperature change [CSV file]. Retrieved from
    https://www.kaggle.com/datasets/sevgisarac/temperature-change?select=Environment_Temperature_change_E_All_Data_NOFLAG.csv"),
  p("National Aeronautics and Space Administration (NASA). (2022). NASA Exo-Planetary System [CSV File].
    Retrieved from https://www.kaggle.com/datasets/sathyanarayanrao89/nasa-exoplanetary-system"),
  p("Rice Science Olympiad (Rice SOAA). (2021). fires_clean [CSV file]. Retrieved from
    https://www.kaggle.com/datasets/chidmuthu/fires-clean")
)

implic_limits <- tabPanel("Implications and Limitations",
...
find_disc <- tabPanel("Findings and Discussion",
  h3("Findings"),
  p(strong("1. Of the exoplanets we know of, what is the proportion of those exoplanets
                            being habitable? What are some commonalities between the habitable exoplanets?")),
  p("Out of the planets we know of, a very small proportion of them are habitable. When
...
ack_ref <- tabPanel("Acknowledgements & References",
  h3("Acknowledgements"),
...
report <- tabPanel("Report",
                   tabsetPanel(type = "tabs",
                               tabPanel("Intro", brief_intro),
                               tabPanel("Problem & Questions",
                                        problem_questions),
                               tabPanel("Datasets", datasets),
                               tabPanel("Implications & Limitations",
                                        implic_limits),
                               tabPanel("Findings & Discussion", find_disc),
                               tabPanel("Conclusion", conc),
                               tabPanel("Acknowledgement & References",
                                        ack_ref)
                              )
                   )
